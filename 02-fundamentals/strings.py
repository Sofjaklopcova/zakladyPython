...
date= "12. 10. 2020"
print(date.split('. ')[2] + '-' + date.split('. ')[1] + '-' + date.split('. ')[0])
def funkce(s,vyber):
 if vyber == 1:
      return s.replace(' ', '_').lower()
 elif vyber == 2:
     return camelcase.CamelCase().hump(s).replace(' ', '')
 elif vyber == 3:
     return s.replace(' ', '_').lower() + '\n' + camelcase.CamelCase().hump(s).replace(' ', '')

print(funkce("To je promenna v pythonu",3))


def hesla(kolik):
  for i in range(kolik):
    string = ''.join(random.choice(s.ascii_uppercase) for i in range(3))
    string += ''.join(random.choice(s.ascii_lowercase) for i in range(3))
    string += ''.join(random.choice(s.punctuation) for i in range(1))
    string += ''.join(random.choice(s.digits) for i in range(3))
    print(string)
